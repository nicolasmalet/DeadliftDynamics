...
def pos_to_screen(pos: np.ndarray) -> tuple[int, int]:
    ...
    return (
        int((pos[0] - focus[0]) * pixel_per_meter * ratio_screen_reality) + screen_size_x // 2,
...
def draw_bone(screen: pg.Surface, start: np.ndarray, end: np.ndarray, color: tuple[int, int, int]) -> None:
    """Draws a bone on the screen."""
    draw_line(screen, start, end, color)


def draw_muscle(screen: pg.Surface, muscle: Muscle, current: Frame, effort: float) -> None:
...
def draw_head(screen: pg.Surface, current: Frame) -> None:
    """Draws the head of the character."""
    x, y = pos_to_screen(current.ends[2] + r_head * current.e_r[2])
...
def draw_bar(screen: pg.Surface, current: Frame) -> None:
    """Draws the barbell."""
    x, y = pos_to_screen(current.ends[-1])
...
def update_display(model: Model, state: State, efforts: np.ndarray, time: float, screen: pg.Surface, font: pg.font.Font) -> bool:
    ...
    for i, bone in enumerate(model.bones):
        draw_bone(screen, current.origins[i], current.ends[i], bone.color)
    draw_head(screen, current)
